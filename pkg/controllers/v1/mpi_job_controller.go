...
const (
	...
	sshAuthSecretSuffix     = "-ssh-auth"
	sshAuthVolumeName       = "ssh-auth"
...
func (c *MPIJobController) syncHandler(key string) error {
	...
	if !done {
		...
		workersSvc, err := c.getOrCreateWorkersService(mpiJob)
		if err != nil {
			return err
		}

		config, err := c.getOrCreateConfigMap(mpiJob, workersSvc, workerReplicas, isGPULauncher)
		if err != nil {
			return err
		}

		sshSecret, err := c.getOrCreateSSHAuthSecret(mpiJob)
		if err != nil {
			return err
		}
		...
		worker, err = c.getOrCreateWorker(mpiJob, sshSecret, workersSvc)
		if err != nil {
			return err
		}
		if launcher == nil {
			launcher, err = c.kubeClient.CoreV1().Pods(namespace).Create(c.newLauncher(mpiJob, sshSecret, config, isGPULauncher))
			if err != nil {
...
func (c *MPIJobController) getRunningWorkerPods(service *corev1.Service) ([]*corev1.Pod, error) {
...
func (c *MPIJobController) getOrCreateConfigMap(mpiJob *kubeflow.MPIJob, service *corev1.Service, workerReplicas int32, isGPULauncher bool) (*corev1.ConfigMap, error) {
	newCM := newConfigMap(mpiJob, workerReplicas, isGPULauncher)
	podList, err := c.getRunningWorkerPods(service)
	if err != nil {
		return nil, err
	}
	...
	if !reflect.DeepEqual(cm.Data, newCM.Data) {
		cm.Data = newCM.Data
		cm, err = c.kubeClient.CoreV1().ConfigMaps(mpiJob.Namespace).Update(cm)
		if err != nil {
			return nil, err
		}
	}
...
func (c *MPIJobController) getOrCreateSSHAuthSecret(mpiJob *kubeflow.MPIJob) (*corev1.Secret, error) {
	secret, fErr := c.secretLister.Secrets(mpiJob.Namespace).Get(mpiJob.Name + sshAuthSecretSuffix)
	if fErr != nil && !errors.IsNotFound(fErr) {
		return nil, fErr
	}
	...
	if errors.IsNotFound(fErr) {
		return c.kubeClient.CoreV1().Secrets(mpiJob.Namespace).Create(newSecret)
	}
	// If the Secret is not controlled by this MPIJob resource, log a warning
	// and return.
	...
	gotKeys := keysFromData(secret.Data)
	wantKeys := keysFromData(newSecret.Data)
	// If the Secret doesn't have the same keys, update it.
	if !equality.Semantic.DeepEqual(gotKeys, wantKeys) {
		secret.Data = newSecret.Data
		return c.kubeClient.CoreV1().Secrets(mpiJob.Namespace).Update(secret)
	}
	return secret, nil
}
...
func (c *MPIJobController) getOrCreateWorkersService(mpiJob *kubeflow.MPIJob) (*corev1.Service, error) {
	...
	if errors.IsNotFound(err) {
		return c.kubeClient.CoreV1().Services(mpiJob.Namespace).Create(newSvc)
	}
	// If the Service is not controlled by this MPIJob resource, log a warning
	// and return.
	...
	if !equality.Semantic.DeepEqual(svc.Spec.Selector, newSvc.Spec.Selector) {
		...
		return c.kubeClient.CoreV1().Services(mpiJob.Namespace).Update(svc)
	}
	return svc, nil
}

// getOrCreateWorker gets the worker Pods controlled by this MPIJob, or creates
// them if they don't exist.
func (c *MPIJobController) getOrCreateWorker(mpiJob *kubeflow.MPIJob, secret *corev1.Secret, svc *corev1.Service) ([]*corev1.Pod, error) {
	var (
	...
	for ; i < *workerReplicas; i++ {
		...
		if errors.IsNotFound(err) {
			worker := newWorker(mpiJob, secret, svc, name, c.gangSchedulerName)
			if worker == nil {
...
func (c *MPIJobController) doUpdateJobStatus(mpiJob *kubeflow.MPIJob) error {
	...
	if err != nil {
		return fmt.Errorf("updating MPI job status: %w", err)
	}
	return nil
}
...
func newConfigMap(mpiJob *kubeflow.MPIJob, workerReplicas int32, isGPULauncher bool) *corev1.ConfigMap {
	...
	for i := 0; i < int(workerReplicas); i++ {
		workersPrefix := mpiJob.Name + workerSuffix
		buffer.WriteString(fmt.Sprintf("%s-%d.%s slots=%d\n", workersPrefix, i, workersPrefix, slots))
	}
...
func newSSHAuthSecret(mpiJob *kubeflow.MPIJob) (*corev1.Secret, error) {
	...
	return &corev1.Secret{
		...
		Data: map[string][]byte{
			...
			sshAuthPublicKey:         ssh.MarshalAuthorizedKey(publicKey),
		},
	}, nil
}
...
func newWorkersService(mpiJob *kubeflow.MPIJob) *corev1.Service {
	return &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			Selector:  defaultWorkerLabels(mpiJob.Name),
		},
	}
}
...
// sets the appropriate OwnerReferences on the resource so handleObject can
// discover the MPIJob resource that 'owns' it.
func newWorker(mpiJob *kubeflow.MPIJob, secret *corev1.Secret, svc *corev1.Service, name, gangSchedulerName string) *corev1.Pod {
	podTmpl := mpiJob.Spec.MPIReplicaSpecs[kubeflow.MPIReplicaTypeWorker].Template.DeepCopy()

	// keep the labels which are set in PodTemplate
	if len(podTmpl.Labels) == 0 {
		podTmpl.Labels = make(map[string]string)
	}

	for key, value := range svc.Spec.Selector {
		podTmpl.Labels[key] = value
	}
	podTmpl.Spec.Hostname = name
	podTmpl.Spec.Subdomain = svc.Name
	setRestartPolicy(podTmpl, mpiJob.Spec.MPIReplicaSpecs[kubeflow.MPIReplicaTypeWorker])

	if len(podTmpl.Spec.Containers) == 0 {
		klog.Errorln("Worker pod does not have any containers in its spec")
		return nil
	}
	container := podTmpl.Spec.Containers[0]
	if len(container.Command) == 0 && len(container.Args) == 0 {
		container.Args = []string{"/usr/sbin/sshd", "-De"}
	}

	volumes, volumeMounts := podVolumes(secret, nil)
	podTmpl.Spec.Volumes = append(podTmpl.Spec.Volumes, volumes...)
	container.VolumeMounts = append(container.VolumeMounts, volumeMounts...)

	podTmpl.Spec.Containers[0] = container

	// add SchedulerName to podSpec
	if gangSchedulerName != "" {
		if podTmpl.Spec.SchedulerName != "" && podTmpl.Spec.SchedulerName != gangSchedulerName {
			klog.Warningf("%s scheduler is specified when gang-scheduling is enabled and it will be overwritten", podTmpl.Spec.SchedulerName)
		}
		podTmpl.Spec.SchedulerName = gangSchedulerName

		if podTmpl.Annotations == nil {
			podTmpl.Annotations = map[string]string{}
		}
		// we create the podGroup with the same name as the mpijob
		podTmpl.Annotations[podgroupv1beta1.KubeGroupNameAnnotationKey] = mpiJob.Name
	}
	...
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:        name,
			Namespace:   mpiJob.Namespace,
			Labels:      podTmpl.Labels,
			Annotations: podTmpl.Annotations,
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(mpiJob, kubeflow.SchemeGroupVersionKind),
			},
		},
...
func podVolumes(secret *corev1.Secret, config *corev1.ConfigMap) ([]corev1.Volume, []corev1.VolumeMount) {
	volumes := []corev1.Volume{
		{
			Name: sshAuthVolumeName,
			VolumeSource: corev1.VolumeSource{
				Secret: &corev1.SecretVolumeSource{
					SecretName:  secret.Name,
					DefaultMode: newInt32(0666),
...
func (c *MPIJobController) newLauncher(mpiJob *kubeflow.MPIJob, secret *corev1.Secret, config *corev1.ConfigMap, isGPULauncher bool) *corev1.Pod {
	launcherName := mpiJob.Name + launcherSuffix
	...
	volumes, volumeMounts := podVolumes(secret, config)
