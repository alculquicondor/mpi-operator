	...
	for _, action := range actions {
		if len(action.GetNamespace()) == 0 {
			...
			for _, r := range skipResources {
				if action.Matches("list", r) || action.Matches("watch", r) {
					skip = true
					break
				}
			}
...
func TestLauncherNotControlledByUs(t *testing.T) {
	...
	launcher := fmjc.newLauncher(mpiJob, &corev1.Secret{}, &corev1.ConfigMap{}, isGPULauncher(mpiJob))
...
func TestSecretNotControlledByUs(t *testing.T) {
	...
	secret, _ := newSSHAuthSecret(mpiJob)
...
func TestShutdownWorker(t *testing.T) {
	...
	for i := 0; i < int(replicas); i++ {
		...
		worker := newWorker(mpiJob, &corev1.Secret{}, &corev1.Service{}, name, "")
...
func TestLauncherActiveWorkerReady(t *testing.T) {
	...
	for i := 0; i < int(replicas); i++ {
		name := fmt.Sprintf("%s-%d", mpiJob.Name+workerSuffix, i)
		worker := newWorker(mpiJob, secret, service, name, "")
	...
	fmjc := f.newFakeMPIJobController()
	launcher := fmjc.newLauncher(mpiJob, secret, configMap, isGPULauncher(mpiJob))
	launcher.Status.Phase = corev1.PodRunning
	f.setUpLauncher(launcher)
...
func TestWorkerReady(t *testing.T) {
	...
	fmjc := f.newFakeMPIJobController()
	expLauncher := fmjc.newLauncher(mpiJob, secret, configMap, isGPULauncher(mpiJob))
